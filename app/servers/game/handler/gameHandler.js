...
handler.game_process = function(msg,session,next){
	...
	if(process == 'ready'){
		...
		gameLogicRemote.throw(self.app,uid,rid,msg.location,channel,username,channelService);
			...
			gameDao.nextCurPlayer(rid,function(err){
				...
				gameDao.getIsGameNum(rid,function(err,isGameNumArr){
					...
					if(sum<=1){
						...
						gameLogicRemote.restartGame(self.app,uid,rid,channel,channelService,game_winner,username);
