			...
			gameDao.sub_round(rid,round,function(err,my_round){
				gameDao.get_players_location(rid,function(err,locations){
					...
					async.waterfall([
						...
						SJGameLogicRemote.peipai_tips(rid,first_location,function(paix){
							var param = {
								route:'onShoupai',
								paixing:paixing,
								round:my_round,
...
SJGameLogicRemote.peipai_tips = function(rid,location,callback){
	//对牌型进行自动配牌提示
	/*(1)1-2;3-4;
	  (2)1-3;2-4;
	  (3)1-4;2-3;
		...
		gameDao.get_room_by_room_id(rid,function(err,room_info){
			if(local != 0){
				...
				if(is_game_local == 0 || is_game_local == 9){
					SJGameLogicRemote.ready(rid,local,cache,channel,null);
					...
					SJGameLogicRemote.qieguo(rid,local,true,cache,channel,channelService);
				}
			}
		});
	});
