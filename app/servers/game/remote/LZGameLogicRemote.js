			...
			gameDao.sub_round(rid,round,function(err,my_round){
				gameDao.get_players_location(rid,function(err,locations){
					...
					async.waterfall([
						...
						LZGameLogicRemote.peipai_tips(rid,first_location,function(paix){
							var param = {
								route:'onShoupai',
								paixing:paixing,
								round:my_round,
...
LZGameLogicRemote.peipai_tips = function(rid,location,callback){
	//对牌型进行自动配牌提示
	/*(1)1-2;3-4;
	  (2)1-3;2-4;
	  (3)1-4;2-3;
...
LZGameLogicRemote.changeCurPlayer = function(rid,location,status,cache,channel){
	...
	if(status == Code.GAME.PEIPAI){
		LZGameLogicRemote.peipai_tips(rid,location,function(paix){
			param['tip'] = paix;
			utils.pushMessage(rid,channel,param,cache);
		});
	}else{
