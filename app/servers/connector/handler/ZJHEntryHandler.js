	...
	playerDao.getPlayerByPlayerId(playerId,function(err,player){
		playerDao.getFangKa(playerId,function(err,refangka){
			...
			playerDao.subFangKa(playerId,fangKa,function(err,code){
				ZJHGameDao.createRoomByPlayerId(playerId,player.nickName,roomType,totalRound,100, function(err,res){
					...
					console.log('leave create func ......');
					self.app.rpc.game.ZJHGameRemote.add(session,uid, self.app.get('serverId'), rid, true, function(location){
...
handler.enter = function(msg, session, next) {
	...
	ZJHGameDao.returnRoom(roomNum, function(err,res){
...
var onUserLeave = function(app, session) {
	...
	ZJHGameDao.getPlayerLocal(rid,playerId,function(err,location){
		if(location != null){
			app.rpc.game.ZJHGameRemote.kick(session,session.uid,"connector-server-1", session.get('rid'),function(location){
