...
userDao.getUserById = function (userId, cb) {
    console.log('go into userDao.getUserById ', userId);
...
userDao.getUserByPhone = function (phone, cb) {
    console.log('go into userDao.getUserByPhone ', phone);
    var sql  = 'select * from user where phone_num = ?';
    var args = [phone];
    //pomelo.app.get('dbclient').query
    pomelo.app.get('dbclient').query(sql, args, function (err, res) {
        if (err !== null) {
...
userDao.getUserByImei = function (imei, cb) {
    console.log('go into userDao.getUserByImei imei:' + imei);
    ...
    pomelo.app.get('dbclient').insert(sql, args, function (err, res) {
        if (err !== null) {
            ...
            var rs = res[0];
            if (!!rs) {
				console.log('getUserByImei get user succ......');
                utils.invokeCallback(cb, null, new User(rs));
            } else {
...
userDao.createUserByImei = function (imei, cb) {
    ...
    var sql       = 'insert into user(imei,password,loginCount,lastLoginTime) values(?,?,?,?)';
    var loginTime = Date.now();
    var args      = [imei,utils.random6num(), 1, loginTime];
    pomelo.app.get('dbclient').query(sql, args, function (err, res) {
        if (err !== null) {
            ...
            console.log('userDao creater by imei ok  user:' + JSON.stringify(res));
            userDao.getUserById(res.insertId, cb);
        }
    });
};

userDao.createUserByPhone = function (phone,password, cb) {

    var sql       = 'insert into user(phone_num,password,loginCount,lastLoginTime) values(?,?,?,?)';
    var loginTime = Date.now();
    var args      = [phone,password, 1, loginTime];
    pomelo.app.get('dbclient').query(sql, args, function (err, res) {
        if (err !== null) {
            console.log('userDao.createrUser by imei err');
            console.log(err.message);//ER_DUP_ENTRY: Duplicate entry '' for key 'INDEX_ACCOUNT_USERNAME'
...
userDao.updatePlayerInfo = function (playerId, playerInfo, cb) {
    ...
    var args      = [gender, signature, nickName,playerId];
