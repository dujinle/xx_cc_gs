 ...
 * @param cb
 */
playerDao.createPlayerByUserId = function (userId,phone_num,nickName,password,sign_text,sex_type, cb) {
    console.log('cd playerDao.createPlayerByUserId', userId + ' nickname ' + nickName);
    var sql        = 'insert into player(userId,phone_num,password,nickName,signature,gender,createTime,continueLoginDays,gold) value(?,?,?,?,?,?,?,?,?)';
    var createTime = Date.now();
    var args       = [userId,phone_num,password,nickName,sign_text,sex_type, createTime, 1,10000];
...
playerDao.getPlayerByUserId = function (userId, cb) {
    console.log('go into playerDao.getPlayerByUserId ......');
    ...
    pomelo.app.get('dbclient').query(sql, args, function (err, res) {
        if (!!err) {
            ...
            var rs = res[0];
            if (!!rs) {
				console.log('playerDao.getPlayerByUid get player succ......');
                utils.invokeCallback(cb, null, new Player(rs));
            } else {
				console.log('playerDao.getPlayerByUid no found player......');
...
playerDao.updatePlayerInfo = function (playerId,signature, gender, nickName, cb) {
	console.log('go into playerDao.updatePlayerInfo......');
    var sql = 'update player set signature = ?,gender = ?,nickName = ? where playerId = ?';
    var args = [signature, gender, nickName, playerId];
    pomelo.app.get('dbclient').query(sql, args, function (err, res) {
        if (err) {
			console.log('playerDao.updatePlayerInfo failed......' + err);
            utils.invokeCallback(cb, err.message, null);
        } else {
			console.log('playerDao.updatePlayerInfo succ');
...
playerDao.setStatus = function(playerId,status,cb){
    ...
    pomelo.app.get('dbclient').query(sql,args,function(err,res){
        if(!!err){
            console.log('playerDao.setCLDays');
            utils.invokeCallback(cb,err.message,null);
        }else{
            utils.invokeCallback(cb,null,Code.OK);
        }
...
playerDao.getFangKa = function(playerId,cb){

	var sql = 'select fangka from player where playerId=?';
	var args = [playerId];
	pomelo.app.get('dbclient').query(sql,args,function (err,res) {
		if (!!err) {
			console.log(err);
...
playerDao.subFangKa = function(playerId,fangKa,cb){
    var sql = 'update player set fangka = fangka - ? where playerId =?';
    var args =[fangKa,playerId];
    pomelo.app.get('dbclient').query(sql,args,function(err,res){
        if(!!err){
            console.log(err);
            utils.invokeCallback(cb,err.message,null);
...
playerDao.getGold = function(playerId,cb){
	...
	var sql = 'select gold from player where playerId=?';
	var args = [playerId];
	pomelo.app.get('dbclient').query(sql,args,function (err,res) {
		if (!!err) {
			console.log(err);
...
playerDao.subGold = function (playerId,subN,cb) {
    var sql = 'update player p set p.gold = p.gold - ? where playerId=?';
    var args = [subN,playerId];
	console.log("subGold" + JSON.stringify(args));
    pomelo.app.get('dbclient').query(sql,args,function (err,res) {
        if (!!err) {
            console.log(err);
			utils.invokeCallback(cb,err.message,null);
            ...
            if (res.affectedRows>=1){
                //成功
				sql = 'select gold from player where playerId=?';
				args = [playerId];
				pomelo.app.get('dbclient').query(sql,args,function (err,res) {
					if (!!err) {
						console.log(err);
